...
public class EndTurnPhase implements Phase
{
    ...
    public void onValidAction(GameActionHandler handler)
    {
        ...
        if (playerIndex < handler.getGame().getPlayerTableList().size() - 1
                && handler.getGame().getSortedPlayerList().subList(playerIndex + 1, handler.getGame().getPlayersNumber()).stream()
                        .anyMatch(player -> player.isActive() && player.hasPlayedCard()))
        {
            ...
            for (int i = playerIndex + 1; i < handler.getGame().getPlayersNumber(); i++)
            {
                if (handler.getGame().getSortedPlayerList().get(i).isActive())
                {
                    ...
                    handler.getGame().setCurrentPlayerIndexByTable(
                            handler.getGame().getPlayerTableList().indexOf(handler.getGame().getSelectedPlayer().get()));
    ...
    public boolean isLegitAction(GameActionHandler handler, String playerName, BaseGameAction baseAction)
    {
        ...
        Player currentPlayer = handler.getGame().getSelectedPlayer().orElseThrow(() -> new NoSelectedPlayerException("[EndTurnPhase]"));

        if (!currentPlayer.getNickname().equals(playerName))
            throw new WrongPlayerException();
        return baseAction == BaseGameAction.CHARACTER_CARD_ACTION || baseAction == BaseGameAction.PLAY_CHARACTER_CARD
