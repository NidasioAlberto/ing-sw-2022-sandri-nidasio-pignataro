...
public class MoveMotherNaturePhase implements Phase
{
    ...
    public boolean isLegitAction(GameActionHandler handler, String playerName, BaseGameAction baseAction)
    {
        ...
        Player currentPlayer = handler.getGame().getSelectedPlayer().orElseThrow(() -> new NoSelectedPlayerException("[MoveMotherNaturePhase]"));

        if (!currentPlayer.getNickname().equals(playerName))
            throw new WrongPlayerException();
        return baseAction == BaseGameAction.MOVE_MOTHER_NATURE || baseAction == BaseGameAction.CHARACTER_CARD_ACTION
