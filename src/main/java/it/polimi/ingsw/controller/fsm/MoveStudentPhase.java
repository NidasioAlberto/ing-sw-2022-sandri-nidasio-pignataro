...
public class MoveStudentPhase implements Phase
{
    ...
    public boolean isLegitAction(GameActionHandler handler, String playerName, BaseGameAction baseAction)
    {
        ...
        Player currentPlayer = handler.getGame().getSelectedPlayer().orElseThrow(() -> new NoSelectedPlayerException("[MoveStudentPhase]"));

        if (!currentPlayer.getNickname().equals(playerName))
            throw new WrongPlayerException();
        return baseAction == BaseGameAction.MOVE_STUDENT_FROM_ENTRANCE_TO_ISLAND || baseAction == BaseGameAction.MOVE_STUDENT_FROM_ENTRANCE_TO_DINING
                || baseAction == BaseGameAction.CHARACTER_CARD_ACTION || baseAction == BaseGameAction.PLAY_CHARACTER_CARD;
    }
}
